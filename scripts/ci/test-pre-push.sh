...
# contacts a remote, never regenerates or builds packages, and never reads .env or production data.
set -eu
...
helper_src="$root/scripts/ci/pre-push-deletion-only.sh"
...
repo="$fixture/repo"
mkdir -p "$repo/.githooks" "$repo/scripts/ci" "$fixture/bin"
cp "$hook_src" "$repo/.githooks/pre-push"
cp "$helper_src" "$repo/scripts/ci/pre-push-deletion-only.sh"
chmod +x "$repo/.githooks/pre-push" "$repo/scripts/ci/pre-push-deletion-only.sh"
...
for language in typescript python; do
  cat > "$repo/scripts/ci/verify-$language-package.sh" <<'STUB'
#!/bin/sh
step="$(basename "$0" .sh)"
{
  printf '%s|%s' "$step" "$#"
...
STUB
  chmod +x "$repo/scripts/ci/verify-$language-package.sh"
done
chmod +x "$repo/scripts/docpact-gate.sh"
...
expected_gate_trace() {
  ...
  printf 'verify-typescript-package|0\n'
  printf 'verify-python-package|0\n'
}
...
write partial-after-delete "(delete) $zero40 refs/heads/gone $sha1a\n(delete) $zero40\n"
write empty ""
write blank-line "\n"
...
expect_gate "partial second record after deletion runs the full gate" "$fixture/partial-after-delete"
...
expect_failure "TypeScript failure exits 7 and stops before Python" \
  "$fixture/code-update" verify-typescript-package 7 "FIXTURE_FAIL_STEP=verify-typescript-package"
expect_failure "Python failure exits 7" \
  "$fixture/code-update" verify-python-package 7 "FIXTURE_FAIL_STEP=verify-python-package"

# --- helper availability and classifier failure fall back to the gate ------
mv "$repo/scripts/ci/pre-push-deletion-only.sh" "$fixture/classifier-original"
expect_gate "missing classifier falls back to the complete ordered gate" "$fixture/pure-delete"
printf '#!/bin/sh\nexit 42\n' > "$repo/scripts/ci/pre-push-deletion-only.sh"
chmod +x "$repo/scripts/ci/pre-push-deletion-only.sh"
expect_gate "failing classifier falls back to the complete ordered gate" "$fixture/pure-delete"
mv "$fixture/classifier-original" "$repo/scripts/ci/pre-push-deletion-only.sh"
...
if [ "${1:-}" != '--polluted-child' ]; then
  ...
  env GIT_DIR="$foreign/.git" GIT_WORK_TREE="$foreign" GIT_INDEX_FILE="$foreign/.git/index" \
    sh "$root/scripts/ci/test-pre-push.sh" --polluted-child > "$fixture/polluted.log" 2>&1
  polluted_rc=$?
  set -e
